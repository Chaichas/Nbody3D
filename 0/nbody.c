...
void move_particles(particle_t p, const f32 dt, u64 n)
{
  ...
  #pragma omp parallel for schedule(dynamic) // OpenMP assigns one iteration to each thread, Ref: http://www.inf.ufsc.br/~bosco.sobral/ensino/ine5645/OpenMP_Dynamic_Scheduling.pdf
  //
  for (u64 i = 0; i < n; i++)
    {
      //
      ...
      //Ref:https://www.singularis-lab.com/docs/materials/LSA_2016_05_N-Body-Problem.pdf
      #pragma vector aligned //All memory-based instructions in the coprocessor should be aligned to avoid instruction faults, data access is aligned by 64bytes
      
      #pragma omp simd // Ignore all dependencies inside a loop, It can vectorize code legally in some cases that wouldn’t be possible
      ...
      for (u64 j = 0; j < n; j++)
	{
	  ...
	  const f32 d_3_over_2 = d_2 * sqrt(d_2); //11
...
int main(int argc, char **argv)
{
  ...
  const u64 s = sizeof(particle_t) * n * 6;
